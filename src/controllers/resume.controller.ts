...
const resumeController = {
  patchResume: async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const { title, starred, score, sessions, replaceSessions } =
        req.body ?? {};
      ...
      if (Array.isArray(sessions)) {
        if (replaceSessions) {
          (resume as any).sessions = (sessions as any[]).map(
            sanitizeSession
          ) as any;
        } else {
  ...
  deleteSession: async (req: Request, res: Response) => {
    try {
      const { id, sessionKey } = req.params as {
        id: string;
        sessionKey: string;
      };
      const resume = await Resume.findById(id);
      ...
      resume.sessions = resume.sessions.filter(
        (s: any) => s.key !== sessionKey
      );
      if (resume.sessions.length === before)
  ...
  createNewResumeWithSections: async (req: Request, res: Response) => {
    try {
      ...
      if (
        !Object.values(sections || {}).some(
          (s: any) =>
            ...
            s.items.some((item: any) => item?.text?.trim())
        )
      ) {
      ...
      Object.entries(sections || {}).forEach(
        ([key, sectionData]: [string, any]) => {
          if (
            sectionData?.title?.trim() &&
            Array.isArray(sectionData.items) &&
            sectionData.items.length > 0
          ) {
            const validItems = sectionData.items.filter((item: any) =>
              item?.text?.trim()
            );
